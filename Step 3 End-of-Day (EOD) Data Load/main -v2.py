...
def applyLatest(df, type):
    ...
    #quotes
        ...
        (df['event_seq_nb'] == df_grouped['event_seq_nb'])], 'inner')
...
trade_common = spark.read.parquet('data/output/partition=T/*')
trade = trade_common.select('rec_type', 'trade_dt', 'symbol', 'exchange', 'event_tm', 'event_seq_nb', 'arrival_tm', 'trade_pr')
quote_common = spark.read.parquet("data/output/partition=Q/*")
